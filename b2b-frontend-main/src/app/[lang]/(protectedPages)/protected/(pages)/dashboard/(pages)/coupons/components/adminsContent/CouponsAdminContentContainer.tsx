...
import CreateCouponsActionsInAdminContent from "./CreateCouponsActionsInAdminContent";

type Props = {
  queryParams: DefaultQueryParams;
};

export default function CouponsAdminContentContainer({ queryParams }: Props) {
  ...
  useEffect(() => {
    const fetchCoupons = async () => {
      try {
        setIsLoading(true);
        const response = await couponsRequests.adminGetAll({
          privateAxios,
          queryParams,
        });
  ...
  return (
    <>
      <div className="mb-4 flex justify-between items-center">
        ...
        <CreateCouponsActionsInAdminContent />
      </div>
      {isLoading ? (
        <LoadingInDashboard />
      ) : (
