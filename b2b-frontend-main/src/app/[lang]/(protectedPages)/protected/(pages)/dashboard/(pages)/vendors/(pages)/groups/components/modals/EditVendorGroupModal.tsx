...
export default function EditVendorGroupModal({
  isOpen,
  setIsOpen,
  vendorGroup,
}: Props) {
  ...
  const [name, setName] = useState(vendorGroup.name);
  const [description, setDescription] = useState(vendorGroup.description);
  ...
  const editHandler = useCallback(async (data: object) => {
    setIsLoading(true);
    try {
      const response = await vendorsRequests.editVendorGroup({
        privateAxios,
