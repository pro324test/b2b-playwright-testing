"use client";

import React, { useState } from "react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuTrigger,
} from "@/components/ui/dropdown-menu";
import { PiDotsThreeOutlineVerticalFill } from "react-icons/pi";
import { useAppSelector } from "@/redux/config/hooks";
import { MdAddToPhotos, MdDeleteOutline } from "react-icons/md";
import DeleteVendorModal from "./modals/DeleteVendorModal";
import { Vendor } from "@/types/ourApiSepecifc/Vendor";
import AssociateVendroToVendorGroupModeal from "./modals/AssociateVendroToVendorGroupModeal";

type Props = {
  vendor: Vendor;
};

export default function SingleVendorActionsDropdown({ vendor }: Props) {
  const dictionary = useAppSelector(
    (state) => state.dictionarySlice.dictionary
  );
  const authEntity = useAppSelector((state) => state.authSlice.authEntity);
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
  const [isAssociateVendorGroupModalOpen, setIsAssociateVendorGroupModalOpen] =
    useState(false);
  return (
    <>
      <DeleteVendorModal
        vendor={vendor}
        isOpen={isDeleteModalOpen}
        setIsOpen={setIsDeleteModalOpen}
      />
      <AssociateVendroToVendorGroupModeal
        vendor={vendor}
        isOpen={isAssociateVendorGroupModalOpen}
        setIsOpen={setIsAssociateVendorGroupModalOpen}
      />
      <DropdownMenu modal={false}>
        <DropdownMenuTrigger className="">
          <PiDotsThreeOutlineVerticalFill />
        </DropdownMenuTrigger>
        <DropdownMenuContent>
          {/* <DropdownMenuLabel> */}
          {authEntity?.role == "superadmin" ? (
            <button
              className="w-full px-2 p-1 rounded-md flex justify-between items-center gap-2 text-red-600 transition-colors hover:bg-red-600 hover:text-white"
              onClick={() => setIsDeleteModalOpen(true)}
            >
              {dictionary.delete} <MdDeleteOutline />
            </button>
          ) : (
            ""
          )}
          <button
            className="w-full px-2 p-1 rounded-md flex justify-between items-center gap-2 text-blue-600 transition-colors hover:bg-blue-600 hover:text-white"
            onClick={() => setIsAssociateVendorGroupModalOpen(true)}
          >
            Add to group <MdAddToPhotos />
          </button>
          {/* </DropdownMenuLabel> */}
          {/* <DropdownMenuSeparator /> */}
        </DropdownMenuContent>
      </DropdownMenu>
    </>
  );
}
