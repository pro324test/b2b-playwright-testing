...
export default function UserRegisterOtpStep({ phoneNumber }: Props) {
  ...
  const submitHandler = useCallback(
    async ({
      ...
      setIsLoading(true);
      try {
        const response = await signIn(nextAuthIds.verifyRegistration, {
      ...
      >
        <div className={sharedStyles["container"]}>
          ...
          <div className="mt-4 flex justify-between items-center">
            <Link href={routes.userLogin.href({ lang })} className="main-link">
              {dictionary.login}
            </Link>
          </div>
        </div>
